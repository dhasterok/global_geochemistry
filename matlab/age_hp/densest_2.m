...
ind1 = (data.mgo < 18 | rockgroup(data,'all plutonic')) & ~(ind3 | ind4);
% Group 2 - High-magnesian, igneous and sedimentary rocks
%   sans carbonates/carbonatites
ind2 = (data.mgo >= 18 & ~rockgroup(data,'all plutonic')) & ~(ind3 | ind4);
...
ylim([2000 4000])
legend('Group 1 - Low-MgO, igneous', ...
    'Group 1 - Low-MgO, sedimentary', ...
    'Group 2 - High-MgO, igneous', ...
    'Group 2 - High-MgO, sedimentary', ...
    'Group 4 - Carbonates', ...
    'Group 3 - Carbonatite');
...
terms = {'sio2','sio2^-1','Fe_number','maficity','MALI'};
x = [ones(size(data.sio2)) data.sio2 data.sio2.^-1 data.Fe_number data.maficity data.MALI];
[data.density_sio2,m] = compute_density(ind1 & data.sio2 > 0,data,density_obs,x,terms,'SiO2');
data.density_sio2 = x*m;

% density of high-Mg igneous rocks
% ---------------------------
%terms = {'Mg_number','sio2','maficity','cao','al2o3'};
terms = {'mgo'};
data.density_himg = compute_density(ind2,data,density_obs,[],terms,'High-Mg');

% density of carbonatites
% ---------------------------
terms = {'maficity','feo_tot','p2o5'};
data.density_igcarb = compute_density(ind3,data,density_obs,[],terms,'Carbonatite');

% density of carbonates
% ---------------------------
terms = {'sio2','cao','mgo'};
data.density_sedcarb = compute_density(ind4,data,density_obs,[],terms,'Carbonate');
