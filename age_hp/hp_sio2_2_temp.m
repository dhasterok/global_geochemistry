function [hp_corrected_median] = hp_sio2_2_temp(data,age_div,sio2range,felsic_ind,mafic_ind,folder)
...
for i = 1:size(data.sio2,1)
    hp_corrected_median(i,:) = data.hp_origin(i,:).*(10^((cor_val - data.sio2(i,:)).*x(2)));
end
...
for i = 1:(length(sio2range)-1)
    avg_sio2{i} = data.sio2(data.sio2 >= sio2range(i) & data.sio2 < sio2range(i+1) & ~isnan(data.hp_origin)& ~isnan(data.sio2),:);
    avg_hp{i} = data.hp_origin(data.sio2 >= sio2range(i) & data.sio2 < sio2range(i+1) & ~isnan(data.hp_origin)& ~isnan(data.sio2),:);
end
...
ehp = [-2:0.1:2];
n = hist2d(data.sio2,log10(data.hp_origin),esio2,ehp);
...
subplot(4,1,2)
    ...
    [Qageplut_corrected,Qhpplut_corrected] = whisker(avg_age_plut,heat_production_plut_corrected,'Color',[0.5 0.5 0.5],'Scale','log');
    [Qagevolc_corrected,Qhpvolc_corrected] = whisker(avg_age_volc,heat_production_volc_corrected,'Color',[0.5 0.5 0.5],'Scale','log');    
